...
var promQLTemplates = map[string]string{
	...
	"workspace_gpu_power_usage":            `sum by (workspace) (DCGM_FI_DEV_POWER_USAGE{exported_namespace!=""} * on (exported_namespace) group_left(workspace)(label_replace(kube_namespace_labels{$2}, "exported_namespace", "$1", "namespace", "(.*)"))) or on(workspace) max by(workspace) (label_replace(kube_namespace_labels{$2}, "exported_namespace", "$1", "namespace", "(.*)") * 0)`,
	...
	"namespace_gpu_power_usage":            `sum by (namespace) (DCGM_FI_DEV_POWER_USAGE{exported_namespace!=""} * on (exported_namespace) group_left(workspace)(label_replace(kube_namespace_labels{$2}, "exported_namespace", "$1", "namespace", "(.*)"))) or on(namespace) max by(namespace) (label_replace(kube_namespace_labels{$2}, "exported_namespace", "$1", "namespace", "(.*)") * 0)`,
