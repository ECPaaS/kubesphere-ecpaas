...
func AddToContainer(c *restful.Container, factory informers.InformerFactory, k8sclient kubernetes.Interface,
	...
	ws.Route(ws.GET("/metering").
		...
		Param(ws.QueryParameter("level", "Metering level. Current is LevelNamespace").DataType("string").Required(true)).
		...
		Param(ws.QueryParameter("resources_filter", "The namepsace filter consists of a regexp pattern. It specifies which namespace data to return.").DataType("string").Required(true)).
