...
func createVirtualMachine(virtClient kubecli.KubevirtClient, virtzVM *virtzv1alpha1.VirtualMachine) error {
	...
	createdVM, err := virtClient.VirtualMachine(namespace).Create(context.TODO(), kvVM)
	if err != nil {
		klog.Infof(err.Error())
		return err
	}

	createdVM, err = virtClient.VirtualMachine(createdVM.Namespace).Get(context.TODO(), createdVM.Name, &metav1.GetOptions{})
	if err != nil && !errors.IsNotFound(err) {
		klog.Infof(err.Error())
		return err
	}
...
func deleteVirtualMachine(virtClient kubecli.KubevirtClient, namespace string, vm_instance *virtzv1alpha1.VirtualMachine) error {
	err := virtClient.VirtualMachine(namespace).Delete(context.TODO(), vm_instance.Name, &metav1.DeleteOptions{})
	if err != nil && !errors.IsNotFound(err) {
		klog.Infof(err.Error())
		return err
	}
...
func addVolume(vmiName, volumeName, namespace string, virtClient kubecli.KubevirtClient) error {
	...
	err = virtClient.VirtualMachine(namespace).AddVolume(context.TODO(), vmiName, hotplugRequest)
	if err != nil {
		return fmt.Errorf("error adding volume, %v", err)
	}
...
func removeVolume(vmiName, volumeName, namespace string, virtClient kubecli.KubevirtClient) error {
	err := virtClient.VirtualMachine(namespace).RemoveVolume(context.TODO(), vmiName, &kvapi.RemoveVolumeOptions{
		Name: volumeName,
	})
	if err != nil {
